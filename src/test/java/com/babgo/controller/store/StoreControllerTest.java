...
@WebMvcTest(
        ...
        excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = {
                com.babgo.auth.jwtfilter.JwtAuthenticationFilter.class
        })
)
...
class StoreControllerTest {
    ...
    @MockitoBean AuditorAware<String> auditorAware;
    ...
    void setUp() throws Exception {
        ...
        when(testUser.getName()).thenReturn("tester");
        // 불필요 스텁은 빼두는게 안전 (Mockito의 Unnecessary stubbing 예방)
    ...
    void createStore() throws Exception {
        ...
        verify(storeFacade, times(1)).createStore(eq(testUser), any(StoreInfo.Create.class));
    }
    ...
    void updateStore() throws Exception {
        ...
        verify(storeFacade, times(1))
                .updateStore(eq(testUser), eq(storeId), any(StoreInfo.Update.class));
    }
    ...
    void deleteStore_success() throws Exception {
        ...
        verify(storeFacade, times(1)).deleteStore(eq(testUser), eq(storeId));
    }
    ...
    void prepared_success() throws Exception {
        ...
        when(storeFacade.preparedOrder(eq(testUser), eq(orderId))).thenReturn(output);
        ...
        verify(storeFacade, times(1)).preparedOrder(eq(testUser), eq(orderId));
    }
    ...
    void picked_up_success() throws Exception {
        ...
        StoreInfo.OrderStatusResult output = StoreInfo.OrderStatusResult.from(order);
        when(storeFacade.pickedUpOrder(eq(testUser), eq(orderId))).thenReturn(output);
        ...
        verify(storeFacade, times(1)).pickedUpOrder(eq(testUser), eq(orderId));
    }
    ...
    void delivered_success() throws Exception {
        ...
        StoreInfo.OrderStatusResult output = StoreInfo.OrderStatusResult.from(order);
        when(storeFacade.deliveredOrder(eq(testUser), eq(orderId))).thenReturn(output);
        ...
        verify(storeFacade, times(1)).deliveredOrder(eq(testUser), eq(orderId));
    }
}
