...
public class SearchServiceTest extends MockTest {
    ...
    void 카테고리_타입으로_가게_조회_성공(){
        ...
        // when: 조회
        List<Search> result = searchService.getCategorySearch(command);

        // then: 결과와 호출 검증
        Assertions.assertNotNull(result);
        Assertions.assertEquals(2, result.size());
        Assertions.assertSame(expected, result);
        Mockito.verify(searchRepository, Mockito.times(1)).getCategorySearch(command, SearchService.DEFAULT_RADIUS_METER);
    ...
    @Test
    void 카테고리_타입으로_가게_조회_성공_시_반환_값_10개(){
    ...
    void 가게명_검색타입으로_가게_조회_성공() {
        ...
        // when: 카테고리 검색 실행
        List<Search> result = searchService.getStoreSearch(command);
